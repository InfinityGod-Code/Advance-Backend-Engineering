...
public class DeliveryEventListener {
    ...
    public void onDeliveryStarted(
        ...
        log.info("Notification: Delivery started - deliveryId={}, orderId={}, customerId={}",
                event.getDeliveryId(), event.getOrderId(), event.getCustomerId());

        try {
            sendDeliveryNotification(event);
            ack.acknowledge();
            sseEmitterService.broadcast("DeliveryStarted", event);
    ...
    private void sendDeliveryNotification(DeliveryStartedEvent event) {
        log.info("Sending delivery notification to customer={} for delivery={}, order={}",
                event.getCustomerId(), event.getDeliveryId(), event.getOrderId());
    }
}
